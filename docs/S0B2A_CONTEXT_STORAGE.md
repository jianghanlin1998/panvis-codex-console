...
Public Context Item reads validate stored rows structurally, against the durable Project-to-Big-Task-to-Subtask ownership hierarchy, and across linked predecessor and successor supersession history before returning them. Linked history must remain same-scope, complete, and acyclic, with every non-terminal item `SUPERSEDED` and exactly one terminal `ACTIVE` item; malformed linked history fails closed as sanitized malformed stored data. Standalone items without a predecessor or stored successor retain the existing status contract, including standalone `SUPERSEDED`, without requiring a linked history. Caller-provided exact scopes must identify a real matching hierarchy and otherwise fail through the sanitized parent-not-found contract.
