...
Evaluation uses a stable captured structural representation and then parses that captured data canonically. Accessors are not invoked, inconsistent or throwing runtime descriptors fail closed, and security-relevant decision logic does not reread the hostile original after capture. Stable ordinary, null-prototype, frozen, sealed, cloned, and JSON-round-tripped data copies remain valid.

Narrowing deliberately returns the exact original caller references rather than schema-normalized replacement objects so the output remains an identity-preserving subset of the supplied ceiling. Parser-normalizable `sourceReference` and bounded retest text can therefore retain surrounding whitespace on those returned references even though the decision used canonical parsed data. Returned references are not canonicalized compiler input or access evidence; a later trusted integration must parse metadata at its own boundary and must not treat mutable caller objects as provenance authority.
