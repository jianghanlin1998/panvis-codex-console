...
`execution-renew-window <json-file>` separately records an explicitly approved replacement time window for an expired, unused recovery. It uses `/v0/execution/renew-window` with the same boundaries, retains the original approval and usage, and permits only one amendment of at most three hours. Recording the window does not start execution; see the linked execution contract for exact eligibility and replay behavior.
