...
`execution-renew-window <json-file>` records each explicitly approved replacement time window after the current one expires. It uses `/v0/execution/renew-window` with the same boundaries and at most three hours per approved window, retaining the original approval, all historical windows, usage and call counts. Recording time does not start execution or accept an interrupted role. Later windows also work after the previously acknowledged unknown QA; any new unknown remains blocking. See the linked execution contract for exact eligibility and replay behavior.
