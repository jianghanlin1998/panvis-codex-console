...
export const CONSOLE_RESEARCH_TOOL = {
  ...
  inputSchema: { type: "object", properties: {
    ...
    path: { type: ["string", "null"] }, query: { type: ["string", "null"] },
  ...
  }, required: ["kind", "path", "query", "scope", "offset"], additionalProperties: false },
};
...
export function consoleResearch(storage: TaskStorage, projectId: string, defaultScope: ConsoleScope) {
  ...
  return (input: unknown): object => {
    try {
      ...
      if (args.kind === "repository") return { root, head: git(["rev-parse", "HEAD"]).trim(), branch: git(["branch", "--show-current"]).trim(), changes: clean(git(["status", "--short"])).slice(0, 12000), refs: clean(git(["for-each-ref", "--format=%(refname:short) %(objectname)", "refs/heads/"])).slice(0, 12000), capability: "Read-only project investigation is available; execution and live browser/network validation are separate capabilities and must not be invented." };
      if (args.kind === "files") {
        const files = git(["ls-files", "-z", "--cached", "--others", "--exclude-standard"]).split("\0").filter(path => path && !privatePath(path) && (!args.query || path.toLowerCase().includes(String(args.query).toLowerCase())));
      ...
      return { path: rel, text: clean(text.slice(offset, offset + 16000)), nextOffset: offset + 16000 < text.length ? offset + 16000 : null };
