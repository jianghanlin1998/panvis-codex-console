...
function nav() {
  ...
  const projectTree = project => {
    ...
    const renderTask = task => {
      ...
      const children = task.subtasks.map((sub, index) => row(`s:${sub.id}`, `${index + 1}. ${sub.title}`, sub.materialized ? `subtask/${encodeURIComponent(sub.id)}` : `task/${encodeURIComponent(task.id)}/plan`, false, false, sub.status === 'DONE' ? '✓' : sub.stage ? '●' : '○')).join('');
      return row(`t:${task.id}`, name, `task/${encodeURIComponent(task.id)}`, expanded, Boolean(children), task.status === 'DONE' ? '✓' : task.taskKind === 'SMALL_TASK' ? '小任务' : '任务') + (expanded && children ? `<div class="ctc-tree-children">${children}</div>` : '');
    };
...
async function projectPage(id, tab) {
  ...
  const head = `<div class="ctc-crumb">${pill('项目')} / ${escape(record.project.name)}</div>` + heading(record.project.name, '项目目标、任务推进和已确认的决定，都在同一个工作区。', link('＋ 新任务', `new-task/${encodeURIComponent(id)}`, 'ctc-button ctc-primary'));
  ...
  return head + tabs + scopeSettingsHtml(record.settings) + `<div class="ctc-summary"><span class="ctc-label">${escape(record.project.repository.kind === 'PATH' ? record.project.repository.path : record.project.repository.reference)}</span>${button(record.settings.projectClosed ? '重新启用项目' : '标记项目结束', 'project-status')}</div><div id="task-grid">${draftCards(record.drafts)}${taskRows(tasks)}</div>` + (!record.drafts.length && !tasks.length ? empty('还没有任务', '在项目讨论里直接说出目标，Console 可以创建任务草稿并建议拆分。', link('开始讨论', `project/${encodeURIComponent(id)}/discussion`, 'ctc-button ctc-primary')) : '');
}
...
async function draftPage(id) {
  ...
  const suggested = draft.suggestedSubtasks?.length ? `<section class="ctc-surface ctc-spaced"><h3>建议拆分</h3><p class="ctc-small">这是讨论阶段的建议；确认方向后由 Planner 生成正式任务与依赖。</p>${draft.suggestedSubtasks.map((task, index) => `<article class="ctc-contextitem"><strong>${index + 1}. ${escape(task.title)}</strong><p>${escape(task.goal)}</p></article>`).join('')}</section>` : '';
  return head + origin + `<div class="ctc-note"><span>先把方向聊清楚，再确认下面的产品方案。</span>${button('查看方向表单 ↓', 'scroll-direction')}</div>` + discussion + suggested + `<div class="ctc-sectiontitle" id="direction"><h2>产品方向</h2><span class="ctc-label">可直接修改后确认</span></div>` + briefForm(proposal ?? { title: draft.title, goal: draft.goal, scopeIn: [], scopeOut: [], successCriteria: [] }, draft);
}
async function discussionHtml() {
  ...
  return scopeSettingsHtml(context.settings, scope.kind === 'BIG_TASK' || scope.kind === 'SUBTASK') + `<div class="ctc-two ctc-discussion"><section class="ctc-surface"><div class="ctc-subtitle"><h2>${{ PROJECT: '项目讨论', BIG_TASK: '任务讨论', SUBTASK: '小任务讨论', DRAFT: '方向讨论' }[scope.kind]}</h2>${pill('聊天可建草稿 · 方向先对齐')}</div><div id="messages">${messagesHtml(state.turns)}</div><div id="older-messages">${result.hasPrevious ? button('加载更早的对话', 'more-messages') : ''}</div><form class="ctc-composer" data-form="discussion"><textarea name="message" aria-label="讨论内容" maxlength="8000" required placeholder="告诉我目标、让我建任务或拆分任务，也可以指定某项的检查深度…">${escape(state.drafts.get(scopeKey(scope)) ?? '')}</textarea><div class="ctc-composerfoot"><span class="ctc-label">使用当前范围的上下文</span><button class="ctc-button ctc-primary" type="submit" ${state.turns.some(turn => turn.status === 'RUNNING') ? 'disabled' : ''}>发送</button></div></form></section><aside class="ctc-surface"><h3>这次讨论参考什么</h3><div class="ctc-contextitem">当前${scope.kind === 'PROJECT' ? '项目' : '任务'}的目标<span>原始讨论留在各自范围</span></div>${context.items.slice(0, 8).map(item => `<div class="ctc-contextitem">${escape(item.title)}<span>${escape(item.authority === 'HUMAN' ? '你确认的结论' : '已接受的项目记录')}</span></div>`).join('')}<hr class="ctc-rule"><p class="ctc-label">其他任务的原始聊天不会自动带入。影响产品方向的建议，需要你另行确认。</p>${scope.kind !== 'DRAFT' ? `<div class="ctc-spaced">${button('保存一条已确认结论', 'context-dialog')}</div>` : ''}</aside></div>`;
}
function messagesHtml(turns) {
  ...
  return turns.map(turn => `<article class="ctc-message ctc-human"><div class="ctc-messagehead"><span class="ctc-avatar">H</span><strong>你</strong><span class="ctc-label">${escape(timestamp(turn.createdAt))}</span></div><p>${escape(turn.message)}</p></article><article class="ctc-message"><div class="ctc-messagehead"><span class="ctc-avatar">C</span><strong>Console</strong>${pill(label(turn.status), turn.status === 'RUNNING' ? 'ctc-wait' : '')}</div><p>${escape(turn.answer?.reply ?? (turn.status === 'RUNNING' ? '正在思考。你可以切换页面，回复会保存在这里。' : '本轮没有得到可用回复。原消息已保存，可以继续讨论；不会自动重复调用。'))}</p>${(turn.effects ?? []).map(effect => `<div class="ctc-effect">${effect.kind === 'TASK_CREATED' ? link(effect.description + ' →', `draft/${encodeURIComponent(effect.targetId)}`, 'ctc-link') : effect.kind === 'PLAN_REVIEW_CHANGED' ? link(effect.description + ' →', `task/${encodeURIComponent(effect.targetId)}/plan`, 'ctc-link') : escape(effect.description)}</div>`).join('')}${turn.answer?.proposal ? '<p class="ctc-small">已整理产品方向建议，可在方向表单中查看和修改。</p>' : ''}<p class="ctc-small">${turn.status === 'RUNNING' ? '用量统计中' : `本轮用量：${num(turn.usage?.totalTokens)} tokens`}${turn.failureCode ? ` · ${escape(reasons[turn.failureCode] ?? '本轮中断')}` : ''}</p></article>`).join('');
}
...
async function taskPage(id, tab) {
  ...
  const head = `<div class="ctc-crumb">${link(projectName(record.task.projectId), `project/${encodeURIComponent(record.task.projectId)}`, 'ctc-link')} / ${record.sourceDraft?.kind === 'SMALL_TASK' ? '小任务' : '大任务'}</div>` + heading(record.presentation?.title ?? record.task.title, record.task.goal, `${link('后续改进', `new-task/${encodeURIComponent(record.task.projectId)}/${encodeURIComponent(id)}`)}${execution && ['RUNNING'].includes(execution.phase) ? button('暂停推进', 'pause') : execution && record.canResume ? button(execution.phase === 'APPROVED' ? '开始执行' : '继续执行', 'start', true) : ''}`) + tabsHtml('task', id, tab, [['overview', '概览'], ['discussion', '讨论'], ['plan', '计划'], ['tasks', '小任务'], ['results', '成果'], ['context', '上下文']]);
...
async function renderRoute() {
  ...
  try {
    ...
    main.innerHTML = html; mountGraphLayout(); nav();
