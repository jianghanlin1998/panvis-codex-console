...
function scopeSettingsHtml(settings, frozen = false) {
  ...
  return `<form data-form="scope-settings" class="ctc-policy" data-scope="${escape(scope.kind)}" data-id="${escape(scope.id)}" data-revision="${settings.revision}"><div><strong>${frozen ? '后续规划的检查深度' : '检查深度'}</strong><p class="ctc-small">${settings.reviewLevel ? '当前范围单独设置' : `继承${settings.inheritedFrom?.kind === 'BIG_TASK' ? '大任务' : settings.inheritedFrom?.kind === 'PROJECT' ? '项目' : '内置默认'}：${levelName(settings.effectiveReviewLevel)}`}${frozen ? ' · 本次已审计划保留原约定' : ''}</p></div><select name="reviewLevel" aria-label="检查深度"><option value="" ${settings.reviewLevel === null ? 'selected' : ''}>${scope.kind === 'PROJECT' ? '使用内置默认（独立 QA）' : '使用上级设置'}</option>${['LIGHT','STANDARD','THOROUGH'].map(level => `<option value="${level}" ${settings.reviewLevel === level ? 'selected' : ''}>${levelName(level)}</option>`).join('')}</select><button type="submit" class="ctc-button">保存</button><details class="ctc-scope-preferences"><summary>计划检查与用量偏好（下次计划生效）</summary>${preferencesFields(settings.effectivePreferences)}</details></form>`;
}
...
function discussionFailure(code) {
  const cause = {
    ...
    TOOL_ACTION_ATTEMPTED: '模型尝试了当前聊天不支持的工具，本轮已中断。这是 Console 的工具衔接问题，不需要你重写需求。',
    ACTION_CONFLICT: '任务状态已变化，本轮操作未生效。请查看当前任务后继续。',
    INVALID_OUTPUT: '模型回复的格式未能保存，本轮没有应用任务操作。',
...
async function taskPage(id, tab) {
  ...
  if (tab === 'discussion') return head + blockersHtml(record) + (record.sourceDraft ? `<div class="ctc-note">${link('查看开工前的方向讨论', `draft/${encodeURIComponent(record.sourceDraft.id)}`, 'ctc-link')}</div>` : '') + await discussionHtml();
...
function executionFailureExplanation(execution) {
  ...
  if (code === 'TOOL_ACTION_ATTEMPTED') return '模型请求了当前角色没有接入或不允许的操作：' + ({TOOL_WRITE_IN_READ_ONLY:'只读角色尝试改动文件或执行受限命令',TOOL_DYNAMIC_NOT_CONFIGURED:'动态工具未接入此角色',TOOL_MCP_NOT_CONFIGURED:'外部连接器未接入',TOOL_AGENT_NOT_CONFIGURED:'并行 agent 工具未接入',TOOL_TYPE_UNKNOWN:'无法识别的工具类型'}[execution.lastRoleFailure.diagnostics?.protocolCheck] ?? '旧记录未保存具体类型') + '。这不是 token 超限。请在聊天里调整实现方式，或补齐对应工具接入后再恢复。';
  if (code === 'APP_SERVER_PROTOCOL_ERROR') return 'Console 接收模型活动时遇到格式或顺序不一致。具体环节：' + ({AGENT_MESSAGE_LIFECYCLE:'最终回答重复或生命周期冲突', AGENT_MESSAGE_PHASE:'回答阶段无法识别', TERMINAL_ORDER:'结束后仍收到步骤活动', ITEM_STATUS:'工具或回答状态不一致', TURN_IDENTITY:'步骤编号不一致', COMMAND_WORKING_DIRECTORY:'命令工作目录校验', UNCLASSIFIED:'尚未分类'}[execution.lastRoleFailure.diagnostics?.protocolCheck] ?? execution.lastRoleFailure.diagnostics?.protocolCheck ?? '旧记录未保存') + '。已有代码保留；这不是用量超限，也不计作 QA 失败。可重试当前步骤；若再次发生，请保留诊断记录。';
...
async function subtaskPage(id, tab) {
  ...
  if (tab === 'discussion') return head + (['PAUSED','HUMAN_REQUIRED'].includes(record.execution?.phase) ? `<section class="ctc-surface ctc-spaced ctc-blocker"><h2>当前执行需要处理</h2><p>${escape(executionFailureExplanation(record.execution))}</p><p class="ctc-small">恢复设置作用于整个大任务：${escape(record.parent?.title)}。历史代码和检查记录保留。</p>${button('在这里处理大任务中断', 'parent-recovery', true)}</section>` : '') + await discussionHtml();
  if (tab === 'context') return head + await contextHtml();
...
async function act(action) {
  if (action === 'capabilities') { const result = await api('capabilities', {scope:state.scope}); return openModal('当前工具能力', '<p>这里列出已接入的工具与当前设置；实际调用结果以运行记录为准。</p>' + result.capabilities.map(item => `<article class="ctc-contextitem"><h3>${escape(item.title)} · ${escape({CONNECTED:'已接入',WORKFLOW:'按流程使用',ENABLED:'已开启',RESTRICTED:'受限',SELECT_TASK:'选择任务后查看',NOT_CONFIGURED:'未接入'}[item.state])}</h3><p>${escape(item.detail)}</p></article>`).join(''), 'image-close', '关闭'); }
  if (action === 'parent-recovery') { state.task = state.subtask.parentRecord; return openModal('处理大任务中断', `<p>接下来查看并调整整个大任务「${escape(state.task.task.title)}」的恢复设置，可能继续它的后续小任务。还没有更改任何设置。</p>${button('查看大任务恢复操作', 'recovery-review', true)}${button('调整大任务限制', 'adjust-limits')}`, 'image-close', '返回'); }
  ...
  if (action === 'reassess-blocked') {
    const current = await api('task', {bigTaskId: id});
    if (current.execution.unacknowledgedUnknownUsage || current.execution.expiresAt && Date.parse(current.execution.expiresAt) <= Date.now()) {
      state.task = current; openModal('调整后继续核对', adjustmentForm(current.execution), 'adjust-and-continue', '保存调整并继续'); return;
    }
    await api('task-recover-and-start', {bigTaskId: id, planDigest: current.execution.planDigest,
      expectedRevision: current.execution.limitAdjustment?.revision ?? 0, acknowledgeUnknownUsage: false, durationMinutes: null});
    await renderRoute(); notify('已从保留的代码重新核对。旧阻塞与 QA 记录保留。'); return;
  }
