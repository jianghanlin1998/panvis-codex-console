...
- The driver discards the returned sanitized failure code/diagnostics. The failing provider thread is ephemeral; scoped local runtime-log inspection did not recover its exact failure code or usage. Separately confirmed interface defect: the READ_ONLY event tracker rejects all commandExecution events, including read commands needed for engineering QA. This defect is not asserted as the unique cause of the latest failed call without its missing diagnostics. A bounded repair/recovery proposal is private at `.local/STEP9_QA_RECOVERY_PROPOSAL.md`; no QA policy, unknown-usage waiver, new authorization or aggregate increase has been implemented. Hanlin's decision on the proposed 2M known-token ceiling and one explicitly acknowledged unknown QA attempt is pending.
- Daemon exited normally after the stop; lock/session removed. Schema remains 24. The new deadline continues to run; no automatic second renewal or replay is allowed. Step 9 remains OPEN.
